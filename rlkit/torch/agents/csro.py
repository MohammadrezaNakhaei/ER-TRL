...
class CSROSoftActorCritic(OfflineMetaRLAlgorithm):
    def __init__(
        ...
        self.qf1, self.qf2, self.vf, self.c, self.club_model = nets[1:]
    ...
    @property
    def networks(self):
        return self.agent.networks + [self.qf1, self.qf2, self.vf, self.target_vf, self.c, self.club_model]
...
class CSROPrediction(CSROSoftActorCritic):
    def __init__(
            self,
            env,
            train_tasks,
        ...
        self.use_prediction                 = kwargs['use_prediction']  
        self.pred_criterion                 = nn.MSELoss()
        
        self.context_optimizer = optimizer_class(
            list(self.agent.context_encoder.parameters()) + list(self.decoder.parameters()), 
            lr=self.context_lr,
            )
        # state/reward normalizers for better hyperparameters
        normal_idx = self.n_tasks//2-1 # take the middle task
    ...
    @property
    def networks(self):
        return self.agent.networks + [self.qf1, self.qf2, self.vf, self.target_vf, self.c, self.club_model, self.decoder]
    ...
    def _take_step(self, indices, context):
        ...
        if self.use_prediction:
            ...
            prediction_loss = self.prediction_loss_weight * self.pred_criterion(pred, target)
