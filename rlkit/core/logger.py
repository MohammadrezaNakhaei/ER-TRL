...
def save_weights(weights, names):
    ...
    torch.save(weights, names)

def save_itr_data(itr, **kwargs):
    file_name = osp.join(_snapshot_dir, 'data_epoch_%d.csv' % itr)
...
def save_itr_params(itr, params_dict):
    ...
    if _snapshot_dir:
        if _snapshot_mode == 'all':
            file_names = osp.join(_snapshot_dir,  'agent_itr_%d.pth' % itr)
            save_weights(params_dict, file_names)
        elif _snapshot_mode == 'last':
            file_names = osp.join(_snapshot_dir,  'agent.pth' % itr)
            save_weights(params_dict, file_names)
            ...
            if itr % _snapshot_gap == 0:
                file_names = osp.join(_snapshot_dir,  'agent_itr_%d.pth' % itr)
                save_weights(params_dict, file_names)
            file_names = osp.join(_snapshot_dir,  'agent.pth')
            save_weights(params_dict, file_names)
